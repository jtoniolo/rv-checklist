...
**Vault mapping (deploy note):** the Vault keys at `homelab/infra/rv-checklist`
are lowercase, and one of them is *not* a plain lowercase→uppercase rename:

| Vault key (`homelab/infra/rv-checklist`) | Secret / env var key   |
| ---------------------------------------- | ---------------------- |
...
S3 endpoint for attachment storage (`http://10.0.6.16:3900` on the home lab —

...
await esbuild.build({
  ...
  define: {
    ...
    // No API URL is injected any more (ADR-0020): the published image is
    // environment-blind, so the worker reads the API base URL from its own
    // registration URL (`/sw.js?api=<url>`) at runtime instead.
  },
  banner: {

...
into a homelab. The homelab runs **k3s**, uses **CloudNativePG (CNPG)** for
...
- **API** — **NestJS in k3s**, exposed on its own hostname via a **Cloudflare
  Tunnel** (`cloudflared`), exactly like the other homelab services. **No
...
- **Next.js as an edge-Worker BFF** proxying the API over the tunnel — rejected.
  It adds a second deployment model and a Worker→homelab hop for every dynamic
...
- Deployment matches the homelab's established "service in k3s, Cloudflare
...
- SSR renders the shell fast; authenticated content depends on client-side
  fetches to the homelab API, so app availability is tied to homelab uptime

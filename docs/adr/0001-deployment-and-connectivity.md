...
into a home lab. The home lab runs **k3s**, uses **CloudNativePG (CNPG)** for
Postgres, keeps secrets in **Vault**, and already exposes self-hosted services
(Nextcloud, DSM, Mealie, …) to the internet through **Cloudflare Tunnels**, one
...
- **API** — **NestJS in k3s**, exposed on its own hostname via a **Cloudflare
  Tunnel** (`cloudflared`), exactly like the other home lab services. **No
...
- **Next.js as an edge-Worker BFF** proxying the API over the tunnel — rejected.
  It adds a second deployment model and a Worker→home lab hop for every dynamic
...
- Deployment matches the home lab's established "service in k3s, Cloudflare
...
- SSR renders the shell fast; authenticated content depends on client-side
  fetches to the home lab API, so app availability is tied to home lab uptime

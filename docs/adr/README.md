...
| [0031](0031-web-tier-to-k3s-container.md) | The web tier moves to k3s. It leaves the Cloudflare Worker and runs as a container behind the Cloudflare Tunnel, the same as the API: standalone output, the proxy on Node, one environment-blind image, a distroless runtime stage, a separate web chart, and a hard cutover with no Worker kept. This supersedes the web-tier host of 0001 and the web build target of 0020, and amends 0009 and 0018. |

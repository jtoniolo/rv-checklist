...
- *(platform reality, not spec)* On Android, Chrome's scheduler can start the browser process to run the service worker. If Chrome is force-stopped by the user, or an OEM battery manager blocks it, nothing runs until Chrome next starts; the pending registration then fires. No web API survives Android force-stop.
...
- Installed PWA only: "a web app can only use periodic background sync after a person has installed it on their device, and has launched it as a distinct application." Not available in regular tabs. — [Chrome: Periodic Background Sync](https://developer.chrome.com/docs/capabilities/periodic-background-sync)
...
- The spec directs UAs to impose a per-origin minimum gap between events, **defaulting to 12 hours**, plus a cross-origin cap. — [WICG spec](https://wicg.github.io/periodic-background-sync/)
...
- Events are explicitly **not guaranteed** — conditional on permission, network, interval, and engagement. — [WICG spec](https://wicg.github.io/periodic-background-sync/)
...
- Sync-event execution is time-capped; long transfers belong in Background Fetch, not sync handlers. — [Chrome blog](https://developer.chrome.com/blog/background-sync)
...
4. Silent server push, sub-12-hour background polling, and anything after a user force-stop of Chrome (until Chrome next runs).
